...
export function PublicProfilePage() {
    ...
    if (notFound || !profileUser) return (
        <main className="min-h-screen flex items-center justify-center">
            <div className="text-center max-w-sm px-4">
                ...
                <Link to="/books"
                      className="inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium text-white transition-all"
    ...
    const avgRating = userBooks.length > 0
        ? (userBooks.reduce((sum, b) => sum + (b.rating ?? 0), 0) / userBooks.length).toFixed(1)
        : '—';
    ...
    return (
        <main className="container mx-auto px-4 py-8 max-w-5xl">
            ...
            {/* Back */}
            <Link to="/books"
                  className="inline-flex items-center gap-1.5 text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text)] mb-6 transition-colors">
            ...
            <div className="mb-8 rounded-3xl overflow-hidden shadow-sm"
                ...
                <div className="relative h-40 overflow-hidden"
                     style={{ background: 'linear-gradient(135deg, #9a4a1e 0%, #c4724e 50%, #e8a882 100%)' }}>
                    {/* Decorative circles */}
                    ...
                    <div className="absolute top-4 right-20 w-16 h-16 rounded-full opacity-10"
                         style={{ background: 'white' }} />
                    <div className="absolute -bottom-4 left-1/3 w-24 h-24 rounded-full opacity-10"
                         style={{ background: 'white' }} />
                ...
                <div className="px-6 pb-6">
                    <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                        ...
                        <div className="flex items-end gap-5">
                            ...
                            <div className="-mt-14 flex-shrink-0 relative">
                                <div className="w-24 h-24 rounded-2xl flex items-center justify-center shadow-lg ring-4 ring-[var(--color-surface)]"
                                     style={{ background: 'linear-gradient(135deg, var(--color-accent), #c4724e)' }}>
                    ...
                    <div className="mt-5 grid grid-cols-3 gap-3">
                        {[
                            { icon: <BookOpen className="h-4 w-4" />, value: userBooks.length, label: 'Books Listed' },
                            { icon: <span className="text-sm">✅</span>, value: availableBooks, label: 'Available' },
            ...
            <div className="flex items-center justify-between mb-5">
                <div>
                    <h2 className="font-['Playfair_Display'] text-xl font-bold text-[var(--color-text)]">
                        Books by{' '}
                        <span style={{ color: 'var(--color-accent)' }}>@{profileUser.username}</span>
                    </h2>
                    <p className="text-xs text-[var(--color-text-muted)] mt-0.5">
                        {userBooks.length === 0
                            ? 'No books listed yet'
                            : `${userBooks.length} book${userBooks.length !== 1 ? 's' : ''} available for swapping`}
                    </p>
                </div>
                {userBooks.length > 0 && (
            ...
            {userBooks.length === 0 ? (
                <div className="rounded-3xl p-16 text-center"
                    ...
                    <p className="font-['Playfair_Display'] text-lg font-bold text-[var(--color-text)] mb-1">
                        No books yet
                    </p>
                    <p className="text-sm text-[var(--color-text-muted)]">
                        This user hasn't listed any books for swapping.
                    </p>
                </div>
            ) : (

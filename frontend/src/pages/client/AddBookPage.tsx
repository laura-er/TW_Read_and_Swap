...
export function AddBookPage() {
    ...
    const handleChange = (updated: Partial<AddBookFields>) => {
        ...
        const key = Object.keys(updated)[0] as keyof AddBookFields;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        await axiosInstance.post('/api/books', { title: fields.title, author: fields.author, genre: fields.genre, condition: fields.condition, coverUrl: fields.coverUrl, description: fields.description, ownerId: Number(user?.id) || 1 });
        setIsLoading(false);
        navigate('/books');
    };
    ...
    return (
        <div className="mx-auto max-w-2xl px-4 py-10">
            <div className="flex items-center gap-4 mb-8">
                <Link to="/books" className="flex items-center gap-2 text-sm text-[var(--color-text-muted)] hover:text-[var(--color-text)] transition-colors"><ArrowLeft className="h-4 w-4" />{t.addBook.backToCatalog}</Link>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                ...
                <div className="flex items-center justify-end gap-3">
                    <Link to="/books"><Button variant="secondary">{t.common.cancel}</Button></Link>

...
interface AdminBookRowProps { book: Book; onEdit?: (id: string) => void; onDelete?: (id: string) => void; }
...
export function AdminBookRow({ book, onEdit, onDelete }: AdminBookRowProps) {
    ...
    return (
        <tr className="border-b border-[var(--color-border)] hover:bg-[var(--color-surface-alt)]/50 transition-colors">
            ...
            <td className="px-4 py-3 text-sm text-[var(--color-text-muted)]">{genreLabel(book.genre, t)}</td>
            ...
            <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                    <Button size="sm" variant="ghost" onClick={() => onEdit?.(book.id)}>{t.common.edit}</Button>
                    <Button size="sm" variant="danger" onClick={() => onDelete?.(book.id)}>{t.common.delete}</Button>
                </div>
            </td>
        </tr>
    );

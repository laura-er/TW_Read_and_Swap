...
export function AdminBooksTable({ books, onDelete }: { books: Book[]; onDelete: (id: string) => void }) {
    const { t } = useLanguage();
    return (
            ...
            <table className="w-full text-sm">
                ...
                <tbody>{books.map(book => (
                    <tr key={book.id} className="border-t border-[var(--color-border)] hover:bg-[var(--color-surface-alt)]/50 transition-colors">
                        <td className="px-4 py-3"><div className="flex items-center gap-3"><div className="w-8 h-11 rounded overflow-hidden bg-[var(--color-surface-alt)] flex-shrink-0">{book.coverUrl && <img src={book.coverUrl} alt={book.title} className="w-full h-full object-cover" />}</div><div><p className="font-medium text-[var(--color-text)] line-clamp-1">{book.title}</p><p className="text-xs text-[var(--color-text-muted)]">{book.author}</p></div></div></td>
                        ...
                        <td className="px-4 py-3"><Button size="sm" variant="danger" onClick={() => onDelete(book.id)}>{t.books.delete}</Button></td>
                    </tr>
                ))}</tbody>
            </table>

...
export function AdminUsersTable({ users, onBan, onUnban, onDelete }: { users: AdminUser[]; onBan: (id: string) => void; onUnban: (id: string) => void; onDelete: (id: string) => void }) {
                ...
                <tbody>{users.map((user) => {
                    ...
                    return (
                        <tr key={user.id} className="border-t border-[var(--color-border)] hover:bg-[var(--color-surface-alt)]/50 transition-colors">
                            ...
                            <td className="px-4 py-3">{!isAdmin && (user.isBanned ? <Button size="sm" variant="secondary" onClick={() => onUnban(user.id)}>Unban</Button> : <Button size="sm" variant="secondary" onClick={() => onBan(user.id)}>Ban</Button>)}</td>
                            <td className="px-4 py-3">{!isAdmin && <Button size="sm" variant="danger" onClick={() => onDelete(user.id)}>{t.common.delete}</Button>}</td>
                        </tr>
                    );
                })}</tbody>
            </table>

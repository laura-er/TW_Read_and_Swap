...
interface Props { users: AdminUser[]; onBan: (id: string, duration: string, reason: string, message: string) => void; onUnban: (id: string) => void; onDelete: (id: string) => void; }
...
export function AdminUsersTable({ users, onBan, onUnban, onDelete }: Props) {
    ...
    return (
        <>
            <ConfirmDeleteUserModal isOpen={!!deleteTarget} userName={deleteTarget?.name ?? ''} onConfirm={() => deleteTarget && onDelete(deleteTarget.id)} onClose={() => setDeleteTarget(null)} />
            <BanUserModal isOpen={!!banTarget} userName={banTarget?.name ?? ''} onConfirm={(duration, reason, message) => banTarget && onBan(banTarget.id, duration, reason, message)} onClose={() => setBanTarget(null)} />
            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden">
                <table className="w-full text-sm table-fixed">
                    <thead className="bg-[var(--color-surface-alt)]"><tr>
                        <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wide w-[28%]">{t.admin.users}</th>
                        <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wide w-[8%]">Role</th>
                        <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wide w-[8%]">{t.admin.books}</th>
                        <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wide w-[8%]">{t.swaps.mySwaps}</th>
                        <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wide w-[10%]">{t.admin.status}</th>
                        <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wide w-[8%]">Ban</th>
                        <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wide w-[8%]">{t.common.delete}</th>
                    </tr></thead>
                ...
                {users.length === 0 && <p className="text-center py-10 text-[var(--color-text-muted)]">{t.common.noData}</p>}
            </div>
        </>
    );
}
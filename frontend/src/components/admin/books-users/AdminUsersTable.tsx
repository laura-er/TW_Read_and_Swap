        ...
        <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden">
            <table className="w-full text-sm table-fixed">
                ...
                <tr>
                    ...
                    <th className="text-left px-4 py-3 text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wide w-[8%]">Delete</th>
                </tr>
                </thead>
                <tbody>
                {users.map((user) => {
                    ...
                    return (
                        <tr key={user.id}
                            className="border-t border-[var(--color-border)] hover:bg-[var(--color-surface-alt)]/50 transition-colors">
                            ...
                            <td className="px-4 py-3">
                                <Badge variant={isAdmin ? 'accent' : 'default'}>{user.role}</Badge>
                            </td>
                            <td className="px-4 py-3 text-[var(--color-text)]">
                                {isAdmin ? '—' : user.booksCount}
                            </td>
                            <td className="px-4 py-3 text-[var(--color-text)]">
                                {isAdmin ? '—' : user.swapsCompleted}
                            </td>
                            <td className="px-4 py-3">
                                {isAdmin ? (

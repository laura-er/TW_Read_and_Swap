...
const statusVariant: Record<ReportStatus, 'warning' | 'success' | 'default'> = { open: 'success', resolved: 'success', dismissed: 'default' };

interface ReportsTableProps { reports: ReportedIssue[]; onResolve: (id: string, note: string, action: string) => void; onDismiss: (id: string) => void; }
...
export function ReportsTable({ reports, onResolve, onDismiss }: ReportsTableProps) {
    ...
    return (
        <>
            {resolving && <ResolveIssueModal report={resolving} onConfirm={onResolve} onClose={() => setResolving(null)} />}
            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden">
                    ...
                    <tbody>
                        {reports.map((r) => (
                            <tr key={r.id} className="border-t border-[var(--color-border)] hover:bg-[var(--color-surface-alt)]/50 transition-colors">
                                <td className="px-4 py-3"><Badge variant={r.type === 'book' ? 'info' : 'accent'} className="capitalize">{r.type}</Badge></td>
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex flex-col gap-1">
                                        ...
                                        {r.status === 'resolved' && r.resolveNote && <span className="text-[10px] text-[var(--color-text-muted)] italic line-clamp-1" title={r.resolveNote}>{r.resolveNote}</span>}
                                    </div>
                                </td>
                                <td className="px-4 py-3">
                                    {r.status === 'open' && (
                ...
                {reports.length === 0 && <p className="text-center py-10 text-[var(--color-text-muted)]">No reports found.</p>}
            </div>
        </>
    );
}

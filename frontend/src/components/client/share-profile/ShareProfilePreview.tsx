...
export function ShareProfilePreview({ user }: { user: User }) {
    ...
    return (
        <div className="flex flex-col gap-4 p-6 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)]">
            ...
            <p className="text-sm text-[var(--color-text-muted)] -mt-2">{t.share.publicPreviewDesc}</p>
            <div className="rounded-xl overflow-hidden border border-[var(--color-border)]">
                ...
                <div className="px-5 pb-5 bg-[var(--color-bg)]">
                    <div className="-mt-8 mb-3"><Avatar src={user.avatarUrl} name={user.name} size="lg" className="ring-4 ring-[var(--color-bg)]" /></div>
                    ...
                    <div className="flex gap-4 mt-4 text-sm">
                        ...
                        <span className="flex items-center gap-1.5 text-[var(--color-text-muted)]"><ArrowLeftRight className="h-4 w-4 text-[var(--color-accent)]" />{user.swapsCompleted ?? 0} {t.share.swaps}</span>
                        <span className="flex items-center gap-1.5 text-[var(--color-text-muted)]"><Heart className="h-4 w-4 text-[var(--color-accent)]" />{user.favoritesCount ?? 0} {t.share.favoritesLabel}</span>
                    </div>
                </div>
            </div>
        </div>

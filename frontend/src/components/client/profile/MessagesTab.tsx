...
export function MessagesTab() {
    ...
    const typeStyles = {
        warning: { bg: 'bg-yellow-50 border-yellow-200', icon: '⚠️', label: language === 'ro' ? 'Avertisment' : 'Warning', labelColor: 'text-yellow-700' },
        info:    { bg: 'bg-blue-50 border-blue-200',     icon: 'ℹ️', label: 'Info',                                          labelColor: 'text-blue-700'   },
        ban:     { bg: 'bg-red-50 border-red-200',       icon: '🚫', label: language === 'ro' ? 'Suspendare' : 'Ban',        labelColor: 'text-red-700'    },
    };
    ...
    return (
        <div className="flex flex-col gap-3">
            ...
            {messages.map(msg => {
                ...
                return (
                    <div key={msg.id} onClick={() => !msg.isRead && markAsRead(msg.id)}
                        ...
                        <div className="flex flex-col gap-1 flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs text-[var(--color-text-muted)]">{formatFull(msg.createdAt)}</span>
                            </div>

    ...
    return (
        <div className="rounded-2xl p-5" style={{ background: 'var(--lib-card)', border: '1px solid var(--lib-border)', backdropFilter: 'blur(16px)' }}>
            <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs font-semibold text-green-600 bg-green-100 px-2.5 py-1 rounded-full">{t.swaps.accepted}</span>
            </div>
    ...
    if (completed.length === 0) {
        return (
            <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-12 text-center">
                <div className="w-14 h-14 rounded-full bg-[var(--color-surface-alt)] flex items-center justify-center mx-auto mb-4"><svg className="w-7 h-7 text-[var(--color-text-muted)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg></div>
                <h3 className="font-['Playfair_Display'] font-bold text-[var(--color-text)] mb-1">{t.profile.swapHistoryTitle}</h3>
                <p className="text-sm text-[var(--color-text-muted)]">{t.profile.noSwapHistory}</p>
            </div>
        );
    }

...
export function FavoriteRow({ book, isLast, onRemove }: { book: Book; isLast: boolean; onRemove: (id: string) => void }) {
    ...
    return (
        <tr style={{ borderBottom: isLast ? 'none' : '1px solid var(--lib-border)' }} className="group transition-colors">
            ...
            <td className="hidden px-4 py-3 md:table-cell"><span className="rounded-md px-2 py-0.5 text-xs" style={{ border: '1px solid var(--lib-border)', color: 'var(--lib-text-muted)' }}>{genreLabel(book.genre, t)}</span></td>

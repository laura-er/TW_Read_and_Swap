...
export function BookDetailAddReview({ onSubmit }: BookDetailAddReviewProps) {
    ...
    const ratingLabels: Record<number, string> = {
        1: t.books.ratingPoor,
        2: t.books.ratingFair,
        3: t.books.ratingGood,
        4: t.books.ratingVeryGood,
        5: t.books.ratingExcellent,
    };
            ...
            <div className="flex items-center justify-center gap-2 mb-6">
                {[1,2,3,4,5].map((star) => (
                    <button key={star} type="button" onClick={() => setRating(star)}
                        className={`w-10 h-10 rounded-lg transition-all duration-300 flex items-center justify-center hover:scale-110 ${star <= rating ? 'bg-yellow-400 text-white shadow-lg' : 'bg-(--color-surface-alt) text-(--color-text-muted) border border-(--color-border) hover:bg-(--color-border)'}`}>
                ...
                {rating > 0 && <span className="ml-2 text-sm font-semibold text-(--color-accent)">{ratingLabels[rating]}</span>}
            </div>
            <textarea ref={commentRef} placeholder={t.books.shareThoughts} rows={4}

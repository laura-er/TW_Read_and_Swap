...
export function BookDetailReviewStats({ reviews, baseRating, baseReviewCount }: BookDetailReviewStatsProps) {
    ...
    const recommendPercent = reviews.length > 0
        ...
        : baseRating ? Math.round(((baseRating - 1) / 4) * 100) : 0;
    ...
    return (
        <div className="bg-(--color-surface) p-6 rounded-2xl shadow-xl border border-(--color-border)">
            <h2 className="text-2xl font-bold text-(--color-text) mb-6">Rating Overview</h2>
                ...
                <div className="flex flex-col gap-1.5 flex-1 justify-center">
                    {distribution.map(({ star, pct }) => (
                        <div key={star} className="flex items-center gap-2">
                            ...
                            <div className="flex-1 h-2 rounded-full bg-(--color-surface-alt) overflow-hidden">
                                <div

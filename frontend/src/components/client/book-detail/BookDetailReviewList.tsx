                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                                <div className="flex items-start justify-between mb-2 gap-4">
                                    <div>
                                        <button onClick={() => navigate(`/profile/${review.author.username}`)} className="text-base font-bold text-(--color-text) hover:underline cursor-pointer bg-transparent border-0 p-0">
                                            {review.author.name}
                                        </button>
                                        <p className="text-sm text-(--color-text-muted)">{formatRelative(review.createdAt)}</p>
                                    </div>
                                    <div className="flex items-center gap-0.5 flex-shrink-0">
                                        {Array.from({ length: 5 }).map((_, i) => <svg key={i} className={`w-4 h-4 ${i < review.rating ? 'fill-yellow-400 text-yellow-400' : 'fill-(--color-border) text-(--color-border)'}`} viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>)}
                                    </div>
                                </div>

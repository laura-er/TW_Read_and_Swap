...
interface BookDetailInfoProps { book: Book; averageRating: string; reviewCount: number; }
            ...
            <div className="rounded-2xl border border-(--color-border) bg-(--color-surface) p-4">
                ...
                <div className="flex items-center gap-2 flex-wrap p-3 bg-(--color-surface-alt) rounded-xl">
                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-yellow-400/10 rounded-lg border border-yellow-400/20">
                        <div className="flex gap-0.5">{Array.from({ length: 5 }).map((_, i) => <svg key={i} className="w-3.5 h-3.5 fill-yellow-400" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>)}</div>

...
export function SwapBookInfo({ book }: { book: Book }) {
    ...
    return (
        <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 sticky top-24">
            <h2 className="font-['Playfair_Display'] text-lg font-bold text-[var(--color-text)] mb-4">{t.swaps.bookYoureRequesting}</h2>
            <img src={book.coverUrl} alt={book.title} className="w-full h-56 object-cover rounded-xl mb-4" />
